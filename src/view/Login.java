...
public class Login  {
	...
	//Constructor with method that reads all customer details from text file, in class CustomerData
	public Login (){
		customers = CustomerData.AllCustomerDetails();
	}
	
	//Boolean method that takes input from customer, checks if the password and username are in the Arraylist
	...
	public boolean logInCustomer(){
		...
		do {
			...
			for (int i = 0; i < customers.size(); i++) {
				if (userName.equals(this.customers.get(i).getUserName()) && password.equals(this.customers.get(i).getPassword())) {
					System.out.println("\nYou are logged in!");
					loggedIn = true;
				}
			}
	...
	//Boolean method for logging in employee. Prompts employee for user name and password and checks if user name and password are correct
	public boolean loginEmployee(){
		...
		while( loggedInE != true) {
			...
			if (inputUserName.matches(myEmployee.makeUserNameE()) && inputPassword.matches(myEmployee.makePasswordE())){
